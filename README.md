...
- 📋 **侧边栏每会话消耗显示**：Web 左侧会话列表的每个会话行「标题」与时间之间显示该会话累计消耗金额（浅边框小胶囊，如 `¥0.82`），悬停可看模型与缓存/输入/输出 token 明细；菜单里可开关「会话消耗」
...
登录 DSH Web 后自动生效：左侧会话列表每个会话行显示累计消耗金额徽标（标题与相对时间之间，浅边框小胶囊）。
...
- 说明：金额按 DeepSeek 官方定价的 token 用量精确换算，与真实 API 计费可能存在毫分级差异（峰谷边界顿、不足百万分之 0.01 元）
...
curl http://127.0.0.1:3080/dsh-whale/session-costs.json
```

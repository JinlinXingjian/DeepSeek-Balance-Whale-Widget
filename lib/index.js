...
var css = [
  ...
  // 侧边栏每会话消耗徽标：浅边框小胶囊，插在会话行「标题」与「相对时间」之间；
  // 悬停行时随时间为操作按钮让位
  '.dshwv-cost{flex:none;font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary,#9aa0a6);border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.12));background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.03));border-radius:9px;padding:0 5px;margin:0 2px 0 4px;white-space:nowrap}',
  '.dshwv-cost-row:hover .dshwv-cost{display:none}'
].join('\\n')
...
var sessionCostToggle = document.createElement('input')
sessionCostToggle.type = 'checkbox'
sessionCostToggle.className = 'dshwv-check'
sessionCostToggle.checked = true
sessionCostToggle.title = '在侧边栏会话列表显示每个会话的消耗金额'
sessionCostToggle.addEventListener('change', function () { setSessionCostOn(sessionCostToggle.checked) })
var row8 = menuRow()
row8.appendChild(menuLabel('会话消耗'))
row8.appendChild(sessionCostToggle)
var row9 = menuRow()
...
var scrollGapPx = 17
// 侧边栏会话消耗显示开关：localStorage 持久化（不依赖服务端 size.json 字段）
var sessionCostOn = true
try { sessionCostOn = localStorage.getItem('dshw-session-cost') !== '0' } catch (err) {}
sessionCostToggle.checked = sessionCostOn
function saveConfig() {
  try {
...
function setSessionCostOn(v) {
  sessionCostOn = !!v
  sessionCostToggle.checked = sessionCostOn
  try { localStorage.setItem('dshw-session-cost', sessionCostOn ? '1' : '0') } catch (err) {}
  if (sessionCostOn) {
    applySessionBadges()
    fetchSessionCosts(true)
  } else {
    clearSessionBadges()
  }
}
...
// —— 侧边栏每会话消耗显示 ——
// 金额由宿主侧按「每事件真实模型 + 事件时刻峰谷」精确计价（/dsh-whale/session-costs.json），
// 本端只负责轮询、换算显示格式、并把徽标注入会话行「标题」与「相对时间」之间。
var COSTS_URL = '/dsh-whale/session-costs.json'
var DSHW_BUILD = '0.2.11'
function fmtCost(c) {
...
function fmtModels(models) {
  if (!models) return '模型未知'
  var names = Object.keys(models)
  if (names.length === 0) return '模型未知'
  if (names.length === 1) return '模型 ' + names[0]
  return '多模型 ' + names.slice(0, 2).join(' / ') + (names.length > 2 ? ' 等' : '')
}
var costBySession = {}
var costsFetching = false
var costRefreshTimer = null
var costApplyBusy = false
var costApplyQueued = false
var costObserver = null
var costObservedRoot = null
// 相对时间文本（zh/en）：会话行的时间 span 内容即这个形状
var COST_TIME_RE = /^(刚刚|now|\\d+\\s*(分钟|小时|天|个月|年|min|h|d|mo|y)(前)?)$/
function costRowHasTimeChild(el) {
  var kids = el.children
  for (var i = 0; i < kids.length; i++) {
    var k = kids[i]
    if (k.classList && k.classList.contains('dshwv-cost')) continue
    var txt = (k.textContent || '').trim()
    if (COST_TIME_RE.test(txt) && txt.length <= 10) return true
  }
  return false
}
// 行识别：主路径 = 会话行类名（*sessionRow*，当前 DSH 构建 YDXeBa_sessionRow，
// 诊断实测 4 行全中）；兜底 = 时间文本 span 的父元素。排除搜索结果树内的行。
function costRowEls() {
  var rows = []
  var seen = new Set()
  var add = function (el) {
    if (!el || seen.has(el)) return
    if (!costRowHasTimeChild(el)) return
    // 排除搜索结果树（aria-label 含 搜索/Search）
    try {
      var tree = el.closest('[role="tree"]')
      var label = tree ? (tree.getAttribute('aria-label') || '') : ''
      if (label.indexOf('搜索') !== -1 || label.indexOf('Search') !== -1) return
    } catch (err) {}
    seen.add(el)
    rows.push(el)
  }
  var byClass = document.querySelectorAll('[class*="sessionRow"]')
  for (var i = 0; i < byClass.length; i++) add(byClass[i])
  if (rows.length === 0) {
    var spans = document.querySelectorAll('span')
    for (var j = 0; j < spans.length; j++) {
      var s = spans[j]
      if (!s || s.children.length !== 0) continue
      if (s.classList && s.classList.contains('dshwv-cost')) continue
      var txt = (s.textContent || '').trim()
      if (!COST_TIME_RE.test(txt) || txt.length > 10) continue
      var row = s.parentElement
      if (!row || (row.tagName !== 'DIV' && row.tagName !== 'BUTTON' && row.tagName !== 'LI')) continue
...
// 通过 React fiber 取会话 id（会话行的 memoizedProps.node.id 在渲染函数 fiber 上）
function costSessionIdOf(el) {
  for (var k in el) {
    ...
    while (f && guard++ < 40) {
      ...
      if (p && p.node && typeof p.node.id === 'string' && typeof p.node.updatedAt === 'number' && p.node.id.indexOf('session-') === 0) {
        return p.node.id
      }
      f = f.return
    }
...
function fetchSessionCosts(done) {
  if (!sessionCostOn) {
    if (done) done(0)
    return
  }
  if (costsFetching) return
  costsFetching = true
  try {
    fetch(COSTS_URL, { cache: 'no-store' })
      .then(function (r) { return r.json() })
      .then(function (d) {
        if (d && d.ok && d.costs && typeof d.costs === 'object') {
          costBySession = d.costs
          applySessionBadges()
          // 侧边栏可能在脚本启动后才渲染：每次取数后补一次观察器挂载
          if (!costObserver) startCostObserver()
        }
        if (done) done(d && d.ok && typeof d.pending === 'number' ? d.pending : 0)
      })
      .catch(function () { if (done) done(0) })
      .then(function () { costsFetching = false }, function () { costsFetching = false })
  } catch (err) {
    costsFetching = false
    if (done) done(0)
  }
...
function scheduleCostRefresh() {
  ...
  costRefreshTimer = setTimeout(function () {
    costRefreshTimer = null
    fetchSessionCosts()
  }, 1500)
}
// —— id↔行 映射的兜底：从 DSH 官方 session.list 拿「会话标题表」，
// fiber 拿不到 id 时按行标题匹配（标题大多唯一；双标题重复时只影响该行）
var costTitleMap = null // { sessionId: title }
var costTitleToId = null // { title: sessionId }
function loadCostTitleMap() {
  if (costTitleMap) return Promise.resolve(costTitleMap)
  try {
    return fetch('/api/session.list', {
      ...
      .then(function (r) { return r.json() })
      .then(function (d) {
        var v = d && d.result && d.result.ok ? d.result.value : null
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          m[it.sessionId] = typeof title === 'string' ? title : ''
        }
        costTitleMap = m
        return m
      })
      .catch(function () { costTitleMap = {}; return costTitleMap })
  } catch (err) {
    costTitleMap = {}
    return Promise.resolve(costTitleMap)
  }
}
function costIdFromTitle(el) {
  // 行内第一个非时间文本的纯文本 span = 标题
  var kids = el.children
  var titleText = null
  for (var i = 0; i < kids.length; i++) {
    var k = kids[i]
    if (!k || k.tagName !== 'SPAN' || k.children.length !== 0) continue
    if (k.classList && k.classList.contains('dshwv-cost')) continue
    var txt = (k.textContent || '').trim()
    if (COST_TIME_RE.test(txt) && txt.length <= 10) continue
    if (txt.length > 0) { titleText = txt; break }
  }
  ...
  if (!costTitleToId) {
    ...
    var m = costTitleMap || {}
    for (var id in m) {
      if (m[id] && costTitleToId[m[id]] === undefined) costTitleToId[m[id]] = id
    }
  }
  return costTitleToId[titleText] || null
}
function costSessionIdOf(el) {
  // 主路径：React fiber（精确）
  for (var k in el) {
  ...
  // 兜底：按标题匹配（首次用到时拉取一次 title 表）
  if (costTitleMap !== null) return costIdFromTitle(el)
  return null // 标题表异步加载中；apply 队列里稍后重试
}
var costDiagLast = ''
var costLastRows = -1
function reportCostDiag(payload) {
  costLastRows = typeof payload.rows === 'number' ? payload.rows : costLastRows
  var sig = payload.rows + '|' + payload.mapped + '|' + payload.badges + '|' + payload.note
  if (sig === costDiagLast) return
  costDiagLast = sig
  try {
    var body = {
      build: DSHW_BUILD,
      ...
      allRows: payload.allRows || null,
      dom: payload.dom || null,
      note: payload.note,
      at: Date.now()
    }
    fetch('/dsh-whale/cost-diag.json', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function applySessionBadges() {
  if (!sessionCostOn) return
  if (costApplyBusy) { costApplyQueued = true; return }
  costApplyBusy = true
  var diagRows = 0
  var diagMapped = 0
  var diagBadges = 0
  ...
  var diagAllRows = []
  var diagDom = null
  var needTitleFallback = false
  try {
    // 全量行侦查：所有带 sessionRow 类名的元素 + 其直接子 span 原文（看时间文本到底长啥样）
    try {
      var byClass = document.querySelectorAll('[class*="sessionRow"]')
      for (var dc = 0; dc < byClass.length && dc < 10; dc++) {
        var el = byClass[dc]
        var texts = []
        for (var dc2 = 0; dc2 < el.children.length; dc2++) {
          var kk = el.children[dc2]
          texts.push(kk.tagName + ':' + String(kk.textContent || '').replace(/\\s+/g, ' ').slice(0, 24))
        }
        diagAllRows.push({
    ...
    // DOM 全貌：树容器/角色/时间文本统计
    try {
      var treesAll = Array.prototype.map.call(document.querySelectorAll('[role="tree"]'), function (t) {
        return String(t.getAttribute('aria-label') || '')
      })
      var spanAll = document.querySelectorAll('span')
      var timeTexts = []
      for (var st = 0; st < spanAll.length && timeTexts.length < 8; st++) {
        var ss = spanAll[st]
        if (!ss || ss.children.length !== 0) continue
        var stx = (ss.textContent || '').trim()
        if (COST_TIME_RE.test(stx) && stx.length <= 10) timeTexts.push(stx)
      }
      diagDom = {
        treeLabels: treesAll,
    ...
    for (var i = 0; i < rows.length; i++) {
      var row = rows[i]
      diagRows++
      if (diagSample.length < 3) {
        diagSample.push({
          tag: row.tagName,
          cls: String(row.className || '').slice(0, 60),
          kids: Array.prototype.map.call(row.children, function (k) { return k.tagName + '.' + String(k.className || '').slice(0, 30) }).join(',')
        })
      }
      var id = costSessionIdOf(row)
      if (!id) {
        // fiber 失败 → 标题兜底；标题表未就绪则异步加载后重试一次
        if (costTitleMap === null) {
          needTitleFallback = true
          continue
        }
        id = costIdFromTitle(row)
      }
      if (!id) continue
      diagMapped++
      var info = id ? costBySession[id] : null
      var badge = null
      for (var j = 0; j < row.children.length; j++) {
        var ch = row.children[j]
        if (ch.classList && ch.classList.contains('dshwv-cost')) { badge = ch; break }
      }
      if (!info || !(info.amount > 0)) {
        if (badge) badge.parentNode.removeChild(badge)
        row.classList.remove('dshwv-cost-row')
        row.removeAttribute('data-dshwv-cost')
        continue
      }
      diagBadges++
      var text = fmtCost(info.amount)
      var anchor = costTimeAnchorOf(row)
      if (!badge) {
        badge = document.createElement('span')
        badge.className = 'dshwv-cost'
        row.insertBefore(badge, anchor)
      } else if (anchor && badge.nextSibling !== anchor) {
        // React 重排后徽标可能跑位：拉回「标题」与「时间」之间
        row.insertBefore(badge, anchor)
      }
      if (badge.textContent !== text) badge.textContent = text
      badge.title = '本会话消耗 ' + fmtCost(info.amount) + ' · ' + fmtModels(info.models) +
        '（缓存 ' + fmtComp(info.hit) + ' · 输入 ' + fmtComp(info.miss) + ' · 输出 ' + fmtComp(info.out) + ' tokens）'
      row.classList.add('dshwv-cost-row')
      row.setAttribute('data-dshwv-cost', '1')
    }
    if (needTitleFallback) {
      loadCostTitleMap().then(function () {
        setTimeout(function () { costTitleToId = null; applySessionBadges() }, 0)
      })
    }
    reportCostDiag({
      rows: diagRows,
      mapped: diagMapped,
      badges: diagBadges,
      costs: Object.keys(costBySession).length,
      sample: diagSample,
      allRows: diagAllRows,
      dom: diagDom,
      note: needTitleFallback ? 'fiber-miss-title-fallback' : (diagRows > diagMapped ? 'fiber-miss' : 'ok')
    })
  } catch (err) {} finally {
    costApplyBusy = false
    if (costApplyQueued) {
      costApplyQueued = false
      setTimeout(applySessionBadges, 150)
    }
  }
}
function costTimeAnchorOf(row) {
  for (var i = 0; i < row.children.length; i++) {
    var ch = row.children[i]
    if (ch.classList && ch.classList.contains('dshwv-cost')) continue
    var txt = (ch.textContent || '').trim()
    if (COST_TIME_RE.test(txt) && txt.length <= 10) return ch
  }
  return null
}
function clearSessionBadges() {
  var badges = document.querySelectorAll('.dshwv-cost')
  for (var i = 0; i < badges.length; i++) {
    if (badges[i].parentNode) badges[i].parentNode.removeChild(badges[i])
  }
  var rows = document.querySelectorAll('.dshwv-cost-row')
  for (var j = 0; j < rows.length; j++) {
    rows[j].classList.remove('dshwv-cost-row')
    rows[j].removeAttribute('data-dshwv-cost')
  }
}
function startCostObserver() {
  if (costObserver || !window.MutationObserver || !sessionCostOn) return
  // 锚点：会话列表树（aria-label 会话/Sessions）或任一已识别的会话行
  var probe = document.querySelector('[role="tree"][aria-label="会话"], [role="tree"][aria-label="Sessions"]')
  if (!probe) {
    var probeRows = costRowEls()
    if (probeRows.length === 0) return
    probe = probeRows[0]
  }
  // 以侧边栏会话列表根为观察目标（含 --dsh-session-list-edge-inset 自定义属性的容器）；
  // 找不到时退化观察 body
  var root = null
  var p = probe.parentElement
  while (p) {
    try {
      if (getComputedStyle(p).getPropertyValue('--dsh-session-list-edge-inset') !== '') { root = p; break }
    } catch (err) {}
    p = p.parentElement
  }
  costObservedRoot = root || document.body
  costObserver = new MutationObserver(function (muts) {
    if (!sessionCostOn || costApplyBusy) return
    for (var i = 0; i < muts.length; i++) {
      var t = muts[i].target
      if (t && t.nodeType === 1 && t.classList && t.classList.contains('dshwv-cost')) continue
      if (costApplyQueued) return
      costApplyQueued = true
      setTimeout(function () { costApplyQueued = false; applySessionBadges() }, 400)
      return
    }
  })
  costObserver.observe(costObservedRoot, { childList: true, subtree: true })
}
// 轮询：启动后先快后慢——历史会话未扫完（pending>0）或侧边栏行还没识别到
// （costLastRows<=0）时 2 秒一轮，稳定后切 8 秒；Ctrl+F5/重新打开页面都会从头快速补齐
var costPollDelay = 2000
function costPollLoop() {
  fetchSessionCosts(function (pending) {
    costPollDelay = (pending > 0 || costLastRows <= 0) ? 2000 : 8000
  })
  costPollTimer = setTimeout(costPollLoop, costPollDelay)
}
var costPollTimer = setTimeout(costPollLoop, 500)
fetchSessionCosts()
startCostObserver()
})()`
...
function apply(ctx) {
    ...
    // 会话销毁时清理残留聚合与成本缓存，避免内存泄漏
    disposers.push(ctx.on('session/disposed', (session) => {
      if (session && session.id) {
        turnAggs.delete(session.id)
        sessionCostCache.delete(session.id)
      }
    }))

    // —— 侧边栏每会话消耗（模型感知版）——
    // 计价口径与「每轮消耗」完全一致：按每事件真实模型（flash/pro 档）+ 事件时刻
    // 峰谷定价。live 会话直接扫描内存事件表（零落盘）；冷会话经 sessionQuery.load
    // 全量扫描一次后缓存（持久化日志只读，进程期内无需重扫）。
    const sessionCostCache = new Map() // id -> { amount, hit, miss, out, models, eventCount, at, live }
    const sessionCostBusy = new Set() // 正在后台扫描的冷会话 id
    let sessionCostList = { at: 0, records: null }
    const costDiagHistory = [] // 浏览器端徽标诊断历史（保留最近 30 条，GET 返回）

    function costFromEvent(event) {
      if (!event || event.type !== 'assistant/message') return null
      const d = event.data
      if (!d || typeof d !== 'object' || !d.usage) return null
      const usage = d.usage
      const input = Number(usage.inputTokens) || 0
      const cache = Number(usage.cacheReadTokens) || 0
      const output = Number(usage.outputTokens) || 0
      const reasoning = Number(usage.reasoningTokens) || 0
      if (input + cache + output + reasoning === 0) return null
      // 与 每轮消耗 一致：thinking 按输出档计价
      const model = d.message && d.message.source ? String(d.message.source.model || '') : ''
      const p = priceFor(model)
      const pi = isPeakTime(Number(event.time) / 1000) ? 1 : 0
      return {
        cost: (cache / 1e6) * p.hit[pi] + (input / 1e6) * p.miss[pi] + ((output + reasoning) / 1e6) * p.out[pi],
        hit: cache,
        miss: input,
    ...
    function scanSessionEvents(events) {
      let amount = 0
      let hit = 0
      let miss = 0
      let out = 0
      const models = {} // model -> tokens
      for (const ev of events) {
        const r = costFromEvent(ev)
        if (!r) continue
        amount += r.cost
        hit += r.hit
        miss += r.miss
        out += r.out
        if (r.model) models[r.model] = (models[r.model] || 0) + r.hit + r.miss + r.out
      }
      return { amount, hit, miss, out, models }
    }
    async function sessionCostRecords() {
      const now = Date.now()
      if (sessionCostList.records && now - sessionCostList.at < 8000) return sessionCostList.records
      let records = []
      ...
      if (!Array.isArray(records) || records.length === 0) {
        ...
        records = typeof live?.list === 'function' ? live.list().map((s) => ({ header: s.header, live: true, persisted: false })) : []
      }
      sessionCostList = { at: now, records }
      return records
    }
    async function computeSessionCosts(records) {
      const out = {}
      const coldIds = []
      const sessions = ctx.get('sessions')
      for (const rec of records) {
        const id = rec && rec.header ? rec.header.id : null
        if (!id) continue
        if (rec.live) {
          const session = sessions ? sessions.get(id) : undefined
          if (!session) continue
          const cached = sessionCostCache.get(id)
          if (cached && cached.live && cached.eventCount === session.events.length) {
            out[id] = cached
            continue
          }
          const s = scanSessionEvents(session.events)
          const entry = { ...s, eventCount: session.events.length, at: Date.now(), live: true }
          sessionCostCache.set(id, entry)
          out[id] = entry
          continue
        }
        const cached = sessionCostCache.get(id)
        ...
        // 最近 60 秒内失败过的会话：跳过本次（避免每 8 秒重试刷屏），由定时重试路径接管
        const lastErr = sessionCostLastError.get(id)
        if (lastErr && Date.now() - lastErr.at < 60000) continue
        if (sessionCostBusy.has(id)) continue
        sessionCostBusy.add(id)
        coldIds.push(id)
      }
      const canReadCold =
        (() => {
          const p = ctx.get('sessionPersistence')
          if (p && typeof p.inspect === 'function') return true
          const q = ctx.get('sessionQuery')
          return !!(q && typeof q.readSession === 'function')
        })()
      if (coldIds.length > 0 && canReadCold) {
        queueColdScans(coldIds)
      } else {
        for (const id of coldIds) sessionCostBusy.delete(id)
      }
      let pending = 0
      for (const id of sessionCostBusy) {
        if (!sessionCostCache.has(id)) pending++
      }
      return { out, pending }
    }
    // 冷会话后台扫描：每个会话独立排队（一个会话卡住/超时不阻塞其他会话），
    // 并发 2；单次扫描 20 秒超时，失败记日志、60 秒后重试。
    // 读取走 DSH 官方会话服务（与 session.history 同路径）：优先
    // sessionPersistence.inspect（可取消），回退 sessionQuery.readSession。
    const sessionCostLastError = new Map() // id -> { at }
    const SCAN_TIMEOUT_MS = 20000
    async function sessionCostLoadOne(id) {
      const persistence = ctx.get('sessionPersistence')
      if (persistence && typeof persistence.inspect === 'function') {
        const inspected = await persistence.inspect(id, AbortSignal.timeout(SCAN_TIMEOUT_MS))
        if (inspected && inspected.events) return inspected
      }
      const query = ctx.get('sessionQuery')
      if (query && typeof query.readSession === 'function') {
        const loaded = await Promise.race([
          query.readSession(id),
          new Promise((_, reject) => setTimeout(() => reject(new Error('readSession 超时 ' + SCAN_TIMEOUT_MS + 'ms')), SCAN_TIMEOUT_MS)),
        ])
        if (loaded && loaded.events) return loaded
      }
      throw new Error('无可用的会话读取服务（sessionPersistence.inspect / sessionQuery.readSession）')
    }
    async function scanOneCold(id) {
      try {
        const loaded = await sessionCostLoadOne(id)
        const s = scanSessionEvents(loaded.events)
        sessionCostCache.set(id, { ...s, eventCount: loaded.events.length, at: Date.now(), live: false })
        sessionCostLastError.delete(id)
      } catch (err) {
        const prev = sessionCostLastError.get(id)
        if (!prev || Date.now() - prev.at > 600000) {
          console.error('[whale-session-costs] 会话扫描失败: ' + id + ' — ' + String((err && err.message) || err))
          sessionCostLastError.set(id, { at: Date.now() })
        }
      } finally {
        sessionCostBusy.delete(id)
      }
    }
    function queueColdScans(ids) {
      let idx = 0
      let active = 0
      const next = () => {
        while (active < 2 && idx < ids.length) {
          active++
          const id = ids[idx++]
          scanOneCold(id).then(
            () => { active--; next() },
    ...
    // 启动预热：dsh web 启动即后台扫描全部冷会话，用户打开页面时大多已就绪
    ;(async () => {
      try {
        const records = await sessionCostRecords()
        await computeSessionCosts(records)
      } catch (err) {}
    })()
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/cost-diag.json',
      handler: async (req, res) => {
        // 浏览器端徽标自诊断：POST 上报（保留历史，GET 取全部）
        if (req.method === 'POST' || req.method === 'PUT') {
          try {
            const body = await readBody(req)
            const parsed = JSON.parse(body)
            if (parsed && typeof parsed === 'object') {
              costDiagHistory.push(parsed)
              while (costDiagHistory.length > 30) costDiagHistory.shift()
            }
            res.writeHead(200, JSON_HEADERS)
        ...
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify({ ok: true, history: costDiagHistory }))
      },
    }))

    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/session-costs.json',
      handler: async (req, res) => {
        // 侧边栏每会话消耗：live 会话按内存事件表实时计算，冷会话后台扫描缓存；
        // 响应只带已就绪的会话，未就绪的由前端下一轮轮询补齐
        try {
          const records = await sessionCostRecords()
          const result = await computeSessionCosts(records)
          const costs = result.out
          const payload = { ok: true, costs: {}, pending: result.pending }
    ...
    disposers.push(ctx.webServer.tapIndex((html) => {
      if (html.indexOf('/dsh-whale/widget.js?v=') !== -1) return html
      // URL 带版本号：浏览器不会再用旧页面缓存里的旧脚本，重启+F5 必拉新码
      const tag = '<script defer src="/dsh-whale/widget.js?v=0.2.11"></script>'
